...
require("./setting")
const { default: makeWASocket, DisconnectReason, jidDecode, proto, getContentType, useMultiFileAuthState, downloadContentFromMessage, areJidsSameUser, makeCacheableSignalKeyStore, isJidBroadcast } = require("gifted-baileys")
...
function loadSharedCommands() {
  ...
  for (const file of commandFiles) {
    try {
      ...
      if ('name' in command && 'executeWhatsApp' in command) {
        sharedCommands.set(command.name, command);
        console.log(`${c.green}[ SHARED ]${c.r} Loaded command: ${command.name}`);
      }
    } catch (e) {
      console.log(`${c.red}[ SHARED ]${c.r} Failed to load ${file}: ${e.message}`);
    }
  }
}
// ============================================
...
async function startDiscordBot() {
  try {
    ...
    console.log(`${c.red}[ DISCORD ]${c.r} Failed to start Discord bot: ${error.message}`);
  }
}
// ========================================
