...
require("./logger")
require("./setting")
const { downloadContentFromMessage, proto, generateWAMessage, getContentType, prepareWAMessageMedia, generateWAMessageFromContent, GroupSettingChange, jidDecode, WAGroupMetadata, emitGroupParticipantsUpdate, emitGroupUpdate, generateMessageID, jidNormalizedUser, generateForwardMessageContent, WAGroupInviteMessageGroupMetadata, GroupMetadata, Headers, delay, WA_DEFAULT_EPHEMERAL, WADefault, getAggregateVotesInPollMessage, generateWAMessageContent, areJidsSameUser, useMultiFileAuthState, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, makeWaconnet, makeInMemoryStore, MediaType, WAMessageStatus, downloadAndSaveMediaMessage, AuthenticationState, initInMemoryKeyStore, MiscMessageGenerationOptions, useSingleFileAuthState, BufferJSON, WAMessageProto, MessageOptions, WAFlag, WANode, WAMetric, ChatModification, MessageTypeProto, WALocationMessage, ReconnectMode, WAContextInfo, ProxyAgent, waChatKey, MimetypeMap, MediaPathMap, WAContactMessage, WAContactsArrayMessage, WATextMessage, WAMessageContent, WAMessage, BaileysError, WA_MESSAGE_STATUS_TYPE, MediaConnInfo, URL_REGEX, WAUrlInfo, WAMediaUpload, mentionedJid, processTime, Browser, MessageType, Presence, WA_MESSAGE_STUB_TYPES, Mimetype, relayWAMessage, Browsers, DisconnectReason, WAconnet, getStream, WAProto, isBaileys, AnyMessageContent, templateMessage, InteractiveMessage, Header } = require("gifted-baileys")
const os = require('os')
const fs = require('fs')
...
// AI Chat Function with multiple fallbacks
const chatBotAI = async (userMsg, context = '') => {
  const _sys = context || 'You are a helpful WhatsApp bot assistant.'
  ...
  // 2. Fallback: Pollinations OpenAI-compatible
  try {
  ...
  // 3. Final fallback: Pollinations GET with different encoding
  try {
...
ppuser = 'https://dl.dropboxusercontent.com/scl/fi/3zkwwcc84bx4jfqyy6c2r/upgrade.jpg'
}
...
if (global.pmBlocker && !m.isGroup && !isOwner && !isBot && !m.key.fromMe) {
  return reply('Private messages are blocked. Please use the bot in a group.')
}
...
if (m.isGroup && !isAdmins && !isOwner) {
  if (global.antiBadword && budy && global.badWords) {
    let hasBadword = global.badWords.some(w => budy.toLowerCase().includes(w))
    if (hasBadword && isBotAdmins) {
      await X.sendMessage(m.chat, { delete: m.key })
      await X.sendMessage(from, { text: `Watch your language! Badword detected.`, mentions: [sender] })
    }
  }
  if (global.antiTag && m.mentionedJid && m.mentionedJid.length > 5 && isBotAdmins) {
    await X.sendMessage(m.chat, { delete: m.key })
    await X.sendMessage(from, { text: `Mass tagging is not allowed!`, mentions: [sender] })
    return
  }
...
if (fs.existsSync(commandsPath) && isCmd && command) {
  const commandFile = path.join(commandsPath, `${command}.js`);
  if (fs.existsSync(commandFile)) {
    try {
      const cmdModule = require(commandFile);
      if (typeof cmdModule === 'function') {
        await cmdModule(X, m, { from, args, text, isOwner, isGroup, isAdmins, isBotAdmins, reply, reply2 });
      }
    } catch (err) {
      console.error(`Error executing command ${command}:`, err);
      reply('Error executing command: ' + err.message);
    }
  } else {
    // Check for similar commands
    const availableCommands = fs.readdirSync(commandsPath).filter(f => f.endsWith('.js')).map(f => f.replace('.js', ''));
    const matches = didyoumean(command, availableCommands);
    if (matches) {
      reply(`Command not found. Did you mean: *.${matches}*?`);
    }
  }
}
...
if (isCmd) {
  switch(command) {
    case 'ping':
      ...
      await reply('🏓 Pong!');
      const end = Date.now();
      reply2(`Response time: ${end - start}ms`);
      break;
      
    case 'menu':
    case 'help':
      const menuText = `
*${global.botname} - Command Menu*
...
.broadcast <text> - Send to all groups
    ...
    case 'runtime':
      reply(`Bot has been running for: ${runtime(process.uptime())}`);
      break;
      
    case 'ai':
      if (!text) return reply('Please provide a message. Example: .ai hello');
      try {
        const aiResponse = await chatBotAI(text, 'You are UPGRADE IX MD, a helpful WhatsApp bot created by Toosii Tech.');
        reply(aiResponse);
      } catch {
...
// Auto-view status
if (m.mtype === 'statusMessage' && global.autoViewStatus && !m.key.fromMe) {
  try {
    await X.readMessages([m.key]);
    if (global.autoLikeStatus) {
      await X.sendMessage(m.key.remoteJid, { react: { text: '👍', key: m.key } });
    }
  } catch (e) {}
}

} catch (err) {
  console.error('Error in client.js:', err);
}
};
